...
CREATE TABLE `v_all_entries` (`user` int, `date` date, `time` time, `id` int, `table` varchar(21), `type-en` varchar(10), `type-es` varchar(10), `value_en` mediumtext, `value_es` mediumtext);
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_all_entries` AS select `query`.`user` AS `user`,`query`.`date` AS `date`,`query`.`time` AS `time`,`query`.`id` AS `id`,`query`.`table` AS `table`,`query`.`type-en` AS `type-en`,`query`.`type-es` AS `type-es`,`query`.`value_en` AS `value_en`,`query`.`value_es` AS `value_es` from (select `sy`.`id` AS `id`,`sy`.`user` AS `user`,`sy`.`value_en` AS `value_en`,`sy`.`value_es` AS `value_es`,cast(`sy`.`datetime` as date) AS `date`,cast(`sy`.`datetime` as time) AS `time`,'registered_symptom' AS `table`,'Symptom' AS `type-en`,'Sintoma' AS `type-es` from `v_user_symptoms` `sy` union select `sy`.`id` AS `id`,`sy`.`user` AS `user`,`sy`.`value_en` AS `value_en`,`sy`.`value_es` AS `value_es`,cast(`sy`.`datetime` as date) AS `date`,cast(`sy`.`datetime` as time) AS `time`,'registered_feces' AS `table`,'Feces' AS `type-en`,'Heces' AS `type-es` from `v_user_feces` `sy` union select `sy`.`id` AS `id`,`sy`.`user` AS `user`,`sy`.`value_en` AS `value_en`,`sy`.`value_es` AS `value_es`,cast(`sy`.`datetime` as date) AS `date`,cast(`sy`.`datetime` as time) AS `time`,'registered_meal' AS `table`,'Meal' AS `type-en`,'Comida' AS `type-es` from `v_user_meals` `sy` union select `sy`.`id` AS `id`,`sy`.`user` AS `user`,`sy`.`value` AS `value_en`,'' AS `value_es`,cast(`sy`.`datetime` as date) AS `date`,cast(`sy`.`datetime` as time) AS `time`,'registered_supplement' AS `table`,'Supplement' AS `type-en`,'Suplemento' AS `type-es` from `v_user_supplement` `sy`) `query` order by `query`.`date`,`query`.`time`;
